---
title: "Codebook | Entscheidungen des Bundesgerichtshofs in Strafsachen aus dem 20. Jahrhundert (BGH-Strafsachen-20Jhd)"
author: Seán Fobbe
geometry: margin=3cm
papersize: a4
fontsize: 11pt
...
Im Jahr 2022 am BGH eingerichtet sind 13 Zivilsenate, 6 Strafsenate, und 8 Spezialsenate (6 berufsrechtliche Senate, Kartellsenat und Landwirtschaftsenat), sowie ein Großer Zivilsenat, ein Großer Strafsenat und die Vereinigten Großen Senate.\footnote{Geschäftsverteilungsplan des Bundesgerichtshofs für das Jahr 2024.} Ein Senat hat grundsätzlich 7 bis 9 Mitglieder, entscheidet aber als Spruchkörper in Senatsgruppen von 5 Mitgliedern einschließlich des/der Vorsitzenden (§ 139 Abs. 1 GVG). Der 6. Strafsenat bestand zunächst von 1954 bis 1956 und wurde erst 2020 wiedererrichtet.
...
Das Datenmodell dieses Datensatzes ist kompatibel mit dem des Corpus der Entscheidungen des Bundesgerichtshofs (CE-BGH).
...
Dieser Datensatz ist eine digitale Zusammenstellung möglichst aller verfügbaren strafrechtlichen Entscheidungen des deutschen Bundesgerichtshofs (BGH) aus dem 20. Jahrhundert. Enthalten sind Entscheidungen von fünf Strafsenaten aus den Jahren 1950 bis 1999. Für den von 1954 bis 1956 bestehenden 6. Strafsenat liegen keine Daten vor.

Der Datensatz wird in maschinenlesbaren Formaten (TXT und CSV) angeboten, enthält aber auch die PDF-Rohdaten. Wir möchten damit Transparenz in der Herstellung schaffen, aber auch traditionell arbeitenden Rechtswissenschaftler:innen ein für sie technisch vertrautes Angebot machen. Die PDF-Rohdaten wurden inhaltlich nicht verändert und nur die Dateinamen angepasst, um die Lesbarkeit für Mensch und Maschine zu verbessern.
...
\midrule
 ...
 Primäre Datenquelle & Bundesgerichtshof\\
...
Alle in diesem Datensatz enthaltenen Entscheidungen wurden dem Ko-Autor Dr. Tilko Swalve auf Antrag hin vom BGH als Scans auf digitalen Medien übergeben und von den Autoren aufbereitet. Die originalen PDF-Dateien des BGH enthielten den anonymisierten Volltext der Entscheidungen und das Aktenzeichen im Dateinamen. Dem BGH zufolge waren dies alle zum Zeitpunkt der Übergabe am Gericht verfügbare strafrechtliche Entscheidungen aus dem Zeitraum vor 2000.
...
```{r, BGH-Strafsachen-20Jhd_Pipeline_Graph, fig.width = 12, fig.height = 18, fig.pos = "p", fig.cap = "Der vollständige Prozess der Datensatz-Kompilierung."}
...
- Der Datensatz enthält nur begründete Entscheidungen (\emph{publication bias}). Unbegründete Entscheidungen machen den Löwenanteil der Praxis des BGH aus und sind nicht enthalten.
- Erst ab dem 1. Januar 2000 sind begründete Entscheidungen des Bundesgerichtshofs einigermaßen vollständig veröffentlicht (\emph{temporal bias}). Die hier veröffentlichten Entscheidungen sind alles, was der BGH an Dr. Tilko Swalve übergeben wollte bzw. konnte. Wir hatten darüber hinaus keine Möglichkeit die Vollständigkeit zu prüfen.
- Die Digitalisierung mit optischer Zeichenerkennung (OCR) via Tesseract funktioniert sehr gut wenn die Scans gut sind, aber schlecht wenn die Scans schlecht sind (\emph{OCR bias}). Die Qualität der digitalen Texte ist daher für viele Entscheidungen ungenau bis unbrauchbar. Behalten Sie diesen Punkt unbedingt im Hinterkopf bei NLP-Anwendungen, insbesondere bei extrahierten Variablen wie dem im Datensatz enthaltenen Entscheidungsdatum und darauf aufbauenden Variablen (Präsident:innen, usw.).
...
Alle eigenen Beiträge (z.B. durch Zusammenstellung und Anpassung der Metadaten) und damit den gesamten Datensatz stellen die Autor:innen gemäß einer \emph{CC0 1.0 Universal Public Domain Lizenz} vollständig urheberrechtsfrei.
...
Die Dateinamen der PDF- und TXT-Dateien, aber auch alle anderen Metadaten der CSV-Dateien sind kompatibel mit denen des Corpus der Entscheidungen des Bundesgerichtshofs (CE-BGH).\footnote{Fobbe, S. (2024). Corpus der Entscheidungen des Bundesgerichtshofs (CE-BGH) (2024-09-25) [Data set]. Zenodo. \url{https://doi.org/10.5281/zenodo.12814022}}
...
\begin{verbatim}
BGH_Strafsenat-5_NA_1999-06-15_5_StR_614_98_NA_NA_NA.txt
\end{verbatim}
...
Die Qualität der Variablen wurde mit  automatisierten und halb-automatisierten Tests  strikt validiert. Die möglichen Werte der jeweiligen Variablen wurden zudem durch Frequenztabellen und Visualisierungen auf ihre Plausibilität geprüft. Insgesamt werden zusammen mit jeder Kompilierung Dutzende Tests zur Qualitätsprüfung durchgeführt. Alle Ergebnisse der Qualitätsprüfungen sind aggregiert im Compilation Report, dem Codebook, dem Quality Assurance Report und einzeln im Archiv \enquote{ANALYSE} zusammen mit dem Datensatz veröffentlicht.
...
Die PDF-Dokumente wie sie den Autoren durch BGH zur Verfügung gestellt werden, jedoch verbessert durch semantisch hochwertige Dateinamen, die der leichteren Auffindbarkeit von Entscheidungen dienen. Die PDF-Dateien wurden nach Senaten in mehrere ZIP-Archive aufgeteilt um kleinere Downloads zu ermöglichen.
...
Diese Variante enthält die vollständigen aus den PDF-Dateien mittel optischer Zeichenerkennung extrahierten Entscheidungstexte, aber nur einen reduzierten Umfang an Metadaten, der dem der PDF-Dateien entspricht. Die TXT-Dateien sind optisch an das Layout der PDF-Dateien angelehnt. 
...
```{r, BGH-Strafsachen-20Jhd_Density_Zeichen, fig.height = 6, fig.width = 9}
ggplot(data = dt.meta)+
...
```{r,  BGH-Strafsachen-20Jhd_Density_Tokens, fig.height = 6, fig.width = 9}
ggplot(data = dt.meta)+
...
```{r, BGH-Strafsachen-20Jhd_Density_Typen, fig.height = 6, fig.width = 9}
ggplot(data = dt.meta)+
...
```{r, BGH-Strafsachen-20Jhd_Density_Saetze, fig.height = 6, fig.width = 9}
ggplot(data = dt.meta)+
...
```{r, BGH-Strafsachen-20Jhd_Barplot_Spruchkoerper_AZ, fig.height = 6, fig.width = 9}
...
```{r, BGH-Strafsachen-20Jhd_Barplot_Spruchkoerper_DB, fig.height = 12, fig.width = 8}
...
freqtable.praesi <- f.fast.freqtable(dt.meta,
                                 varlist = "praesi",
                                 sumrow = TRUE,
                                 output.list = TRUE,
                                 output.kable = FALSE,
...
```{r, BGH-Strafsachen-20Jhd_Barplot_PraesidentIn, fig.height = 6, fig.width = 9}
...
Hinweis: das Amt Vizepräsident:in wurde am BGH erst 1968 eingerichtet, daher die deutlich höhere Zahl an \enquote{NA} im Vergleich zum Präsident:innenamt.

```{r}

freqtable.vpraesi <- f.fast.freqtable(dt.meta,
                                 varlist = "v_praesi",
                                 sumrow = TRUE,
                                 output.list = TRUE,
                                 output.kable = FALSE,
...
```{r, BGH-Strafsachen-20Jhd_Barplot_VizePraesidentIn, fig.height = 6, fig.width = 9}
...
```{r, BGH-Strafsachen-20Jhd_Barplot_Entscheidungsjahr, fig.height = 6, fig.width = 9}
...
```{r, BGH-Strafsachen-20Jhd_Barplot_EingangsjahrISO, fig.height = 6, fig.width = 9}
...
```{r, BGH-Strafsachen-20Jhd_Density_Dateigroessen_PDF, fig.height = 6, fig.width = 9}
...
```{r, BGH-Strafsachen-20Jhd_Density_Dateigroessen_TXT, fig.height = 6, fig.width = 9}
